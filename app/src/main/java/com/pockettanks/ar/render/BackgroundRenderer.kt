...
/** Renders the live ARCore camera feed as the AR "spatial portal" background. */
class BackgroundRenderer {

    var textureId = -1
        private set

    private val quadCoordsArray = floatArrayOf(-1f, -1f, 1f, -1f, -1f, 1f, 1f, 1f)
    private val quadTexCoordsArray = FloatArray(8)

    private val quadCoordsBuffer: FloatBuffer = directFloatBuffer(8).also {
        it.put(quadCoordsArray); it.position(0)
    }
    private val quadTexCoordsBuffer: FloatBuffer = directFloatBuffer(8)
    ...
    fun draw(frame: Frame) {
        if (frame.hasDisplayGeometryChanged()) {
            ...
            quadTexCoordsBuffer.clear()
            quadTexCoordsBuffer.put(quadTexCoordsArray)
